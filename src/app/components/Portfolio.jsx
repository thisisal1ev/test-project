          ...
          <div className="grid grid-cols-3 gap-2 md:gap-y-4 md:gap-x-7 md:grid-cols-2 lg:grid-cols-3 ">
            {
              portfolio.tools.map((tool) => {
                return <Service stylesForContext={"text-sm leading-5 font-semibold text-black md:text-lg md:leading-7"} stylesForWrapper={"bg-bgWhite rounded-md space-y-3 p-2 md:p-4"} key={tool.id} img={tool.img} context={tool.text} />
              })
            }
          </div>
